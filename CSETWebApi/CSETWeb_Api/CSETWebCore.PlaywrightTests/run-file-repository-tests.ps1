# CSET File Repository E2E Tests Runner (PowerShell)
...
$TestResultsDir = Join-Path $ProjectDir "TestResults\FileRepository"
$CoverageDir = Join-Path $ProjectDir "Coverage\FileRepository"

# Test settings
$TestProject = "CSETWebCore.PlaywrightTests"
$TestFilter = "TestCategory=FileRepository"
$TestAssembly = Join-Path $ProjectDir "bin\$Configuration\net7.0\$TestProject.dll"

# Logging
$LogFile = Join-Path $TestResultsDir "file-repository-tests.log"
$Timestamp = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"

# Functions
function Write-Header {
    Write-Host "==========================================" -ForegroundColor Blue
    Write-Host "  CSET File Repository E2E Tests Runner" -ForegroundColor Blue
    Write-Host "==========================================" -ForegroundColor Blue
    Write-Host ""
}
...
function Invoke-Tests {
    Write-Step "Running File Repository E2E tests..."
    ...
    try {
        ...
        if ($exitCode -eq 0) {
            Write-Success "File Repository tests completed successfully"
        }
        else {
            Write-Error "File Repository tests failed with exit code $exitCode"
        }
...
function New-Reports {
    ...
    $trxFile = Join-Path $TestResultsDir "FileRepositoryTests_$Timestamp.trx"
    if (Test-Path $trxFile) {
        Write-Step "Converting TRX to HTML report..."
        try {
            dotnet tool install --global trx2html
            trx2html $trxFile -o (Join-Path $TestResultsDir "FileRepositoryTests_$Timestamp.html")
            Write-Success "HTML report generated: $(Join-Path $TestResultsDir "FileRepositoryTests_$Timestamp.html")"
        }
        catch {
...
function New-Summary {
    ...
    $summaryFile = Join-Path $TestResultsDir "FileRepositoryTests_$Timestamp.summary.md"
...
- **Test Results:** [TRX File]($TestResultsDir\FileRepositoryTests_$Timestamp.trx)
- **HTML Report:** [HTML Report]($TestResultsDir\FileRepositoryTests_$Timestamp.html)
...
try {
    ...
    Write-Step "Starting File Repository E2E tests at $Timestamp"
    ...
    Write-Step "File Repository E2E tests completed at $(Get-Date)"
    
    if ($testExitCode -eq 0) {
        Write-Success "All File Repository tests passed!"
        Write-Host "==========================================" -ForegroundColor Green
        Write-Host "  File Repository Tests: PASSED" -ForegroundColor Green
        Write-Host "==========================================" -ForegroundColor Green
    }
    else {
        Write-Error "Some File Repository tests failed!"
        Write-Host "==========================================" -ForegroundColor Red
        Write-Host "  File Repository Tests: FAILED" -ForegroundColor Red
        Write-Host "==========================================" -ForegroundColor Red
    }
