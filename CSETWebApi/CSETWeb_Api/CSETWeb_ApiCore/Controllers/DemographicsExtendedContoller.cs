...
{   
    ...
    public class DemographicsExtendedController : ControllerBase
    {
        ...
        /// <summary>
        /// Returns all known metro areas for the specified state.
        /// Uses state FIPS codes to query metro areas dynamically.
        /// </summary>
        ...
        {
            // Get state FIPS code from the state parameter
            var stateFips = GetStateFipsCode(state);
            if (string.IsNullOrEmpty(stateFips))
            {
                return BadRequest($"Invalid state code: {state}");
            }

            var list = _context.METRO_AREA
                .Include(x => x.COUNTY_METRO_AREA)
                .Where(x => x.Metro_FIPS.StartsWith($"{stateFips}-"))
                .ToList();
        ...
        /// <summary>
        /// Gets the FIPS code for a given state.
        /// </summary>
        /// <param name="state">State code or name</param>
        /// <returns>FIPS code for the state</returns>
        private string GetStateFipsCode(string state)
        {
            // State FIPS code mapping
            var stateFipsMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                {"AL", "01"}, {"Alabama", "01"},
                {"AK", "02"}, {"Alaska", "02"},
                {"AZ", "04"}, {"Arizona", "04"},
            ...
            return stateFipsMap.TryGetValue(state, out var fipsCode) ? fipsCode : null;
        }
